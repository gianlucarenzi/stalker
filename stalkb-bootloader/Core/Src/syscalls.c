...
int _write(int file, char *ptr, int len)
{
  ...
  (void)file;
  // Transmit with no timeout, as we'll add a calculated delay afterwards
  status = HAL_UART_Transmit(&huart2, (uint8_t*)ptr, len, HAL_MAX_DELAY);

  // Calculate delay based on baud rate and number of characters
  // 115200 bits/s, 10 bits/char (start + 8 data + stop) = 11520 chars/s
  // Time per char = 1/11520 s = 0.0868 ms/char
  uint32_t delay_ms = (len * 1000) / 11520;

  // Ensure a minimum delay for very short messages or buffer overhead
  if (delay_ms == 0) {
      delay_ms = 1; // Minimum 1ms delay
  }
  HAL_Delay(delay_ms);

  if (status != HAL_OK) len = -1;
  return len;
}
