...
{
  (void)file;
  return LogTask_SendString(ptr, len);
}

/**
 * @brief  Override of HAL_Delay to use RTOS delay when scheduler is running.
 * @note   This ensures that delays are non-blocking when FreeRTOS is active.
 * @note   Falls back to HAL busy-wait delay when scheduler is not running or in ISR context.
...
void HAL_Delay(uint32_t Delay)
{
	...
	if (__get_IPSR() != 0U)
	{
		// In ISR context: use the default HAL busy-wait implementation
		// We cannot use RTOS delay functions from an ISR
		...
		/* Busy Wait while in ISR context */
		while((HAL_GetTick() - tickstart) < wait) { };
	}
	else if (osKernelGetState() == osKernelRunning)
	{
		// RTOS is running and we're in task context: use osDelay (non-blocking)
		osDelay(Delay);
	}
	else
	{
		// RTOS not yet started: use the default HAL busy-wait implementation
		uint32_t tickstart = HAL_GetTick();
		uint32_t wait = Delay;
