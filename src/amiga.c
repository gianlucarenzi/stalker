 ...
 * (C) Copyright 2019/2025 by Gianluca Renzi (RetroBitLab Tech Guy)
 * 
 * //
...
/* KEY definitions moved to usb_hid_keys.h */
...
static const uint8_t scancodeamiga[KEYCODE_TAB_SIZE][2] =
{
	...
	{KEY_HELP,                   0x5F }, // HELP
	{KEY_SCROLL_LOCK,            0x5F }, // HELP (Mapped to ScrollLock?)
	{KEY_APPLICATION,            0x5F }, // APP - HELP (Mapped to Key Application?)
...
led_status_t amikb_send(uint8_t code, int press, int use_OS);
 ...
 **/
uint8_t scancode_to_amiga(uint8_t lkey)
{
...
void amikb_startup(int use_OS)
{
	...
	if (use_OS)
	{
		osDelay(RESET_TIMEOUT_MS * 2);
	}
	else
	{
		HAL_Delay(RESET_TIMEOUT_MS * 2);              // wait for sync
	}
	amikb_send((uint8_t) AMIGA_INITPOWER, 0, use_OS); // send "initiate power-up"
	udelay(200);
	amikb_send((uint8_t) AMIGA_TERMPOWER, 0, use_OS); // send "terminate power-up"
 ...
 **/
led_status_t amikb_send(uint8_t keycode, int press, int use_OS)
{
	int i;
	...
	// Send byte to LED manager for visualization
	extern osMessageQueueId_t ledManagerQueueHandle;
	if (ledManagerQueueHandle != NULL)
	{
		led_manager_message_t led_msg = {
			.command = LED_CMD_BLINK_BYTE,
			.data = keycode,
			.duration_ms = 0
		};
		osMessageQueuePut(ledManagerQueueHandle, &led_msg, 0, 0);
	}
...
#warning "USING TIMED HANDSHAKE"
	if (use_OS)
	{
		osDelay(5); // handshake wait 5msec
	}
	else
	{
...
void amikb_reset(int use_OS)
{
	...
	if (use_OS)
	{
		osDelay(RESET_TIMEOUT_MS + 50);
	}
	else
	{
		HAL_Delay(RESET_TIMEOUT_MS + 50);
	}
 ...
 *         The caller should be take note of the level and timings.
...
/*
 * We can have 2 combinations for RESET:
 * 
