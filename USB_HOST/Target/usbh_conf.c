...
/* Includes ------------------------------------------------------------------*/
#include "usbh_core.h"
#include "main.h" /* for USB Pin & Port definitions */
...
void HAL_HCD_MspInit(HCD_HandleTypeDef* hcdHandle)
{
	...
	if(hcdHandle->Instance==USB_OTG_FS)
	{
		...
		/* GPIOA is USBH_Port, so enable CLK for this port */
		...
		*/
		...
		HAL_GPIO_Init(USBH_Port, &GPIO_InitStruct);
...
void HAL_HCD_MspDeInit(HCD_HandleTypeDef* hcdHandle)
{
	if(hcdHandle->Instance==USB_OTG_FS)
	{
		...
		PA12     ------> USB_OTG_FS_DP
		*/
		HAL_GPIO_DeInit(USBH_Port, USB_DM_Pin|USB_DP_Pin);
...
{
	/* HAL_Delay() is redefined in syscalls.c!
	 * If this is called within an ISR context or without the
	 * RTOS, it falls back to the HAL_Delay-like busy waiting,
	 * ohterwise we are using the RTOS osDelay()
	 */
	HAL_Delay(Delay);
}
